...
import { ClickhouseProgress } from '../../core/states/clickhouse_state';
...
async function main() {
  ...
  const datasource = new SolanaMintStream({
    ...
    logger,
    state: new ClickhouseProgress(clickhouse, {
      table: 'solana_dex_status',
      id: 'mint',
    }),
    onProgress: ({state, interval}) => {
      logger.info({
        message: `${formatNumber(state.current)} / ${formatNumber(state.to)} (${formatNumber(state.percent)}%)`,
